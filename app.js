...
(function(){
  ...
  window.viewQuote = function(qid){
    ...
    modalBody.innerHTML = `
      <div class="quote-print-pro a4">
        <div class="quote-a4-toolbar">
          <button class="pdf-main" onclick="downloadQuotePDF()">حفظ PDF / طباعة</button>
          <button class="wa-main" onclick="sendQuote('${q.id}')">إرسال واتساب</button>
          <button onclick="convertQuoteToOrder('${q.id}')">تحويل إلى طلب</button>
          <button onclick="closeModal()">إغلاق</button>
        </div>

        <div class="quote-a4-page" id="quoteA4Page">
          <div class="a4-head">
            <img class="a4-logo" src="${JMS_REAL_LOGO}" alt="Jeddah Model Industrial Co. Ltd">
            ...
            <div class="a4-title">
              <h2>عرض سعر</h2>
              <p>رقم العرض: ${clean(q.quote_no)}</p>
              <p>تاريخ الإصدار: ${clean(q.date)}</p>
              <p>صالح حتى: ${clean(q.valid_until)}</p>
            </div>
          </div>
          ...
          <div class="a4-info">
            <div class="a4-card">
              ...
              <p>
                <b>اسم العميل:</b> ${customerName(q.customer_id)}<br>
                <b>الجوال:</b> ${clean(c.phone)}<br>
                <b>المدينة:</b> ${clean(c.city)}<br>
                <b>العنوان:</b> ${clean(c.location || c.district)}
              </p>
            </div>
            <div class="a4-card">
              <h3>بيانات العرض</h3>
              <p>
                <b>المندوب:</b> ${repName(q.rep_id)}<br>
                <b>الحالة:</b> ${quoteStatusText(q.status)}<br>
                <b>شروط الدفع:</b> ${clean(q.payment_terms)}<br>
                <b>مدة التسليم:</b> ${clean(q.delivery_terms)}
              </p>
            </div>
          </div>
          ...
          <table class="a4-table">
            ...
            <tbody>
              <tr>
                <td>${clean(q.product)}</td>
                <td>${clean(q.width)} × ${clean(q.length)} ${clean(q.size_unit)}</td>
                <td>${clean(q.thickness)} ${clean(q.thickness_unit)}</td>
                <td>${clean(q.color)}</td>
                <td>${clean(q.material)}</td>
                <td>${clean(q.piece_weight)}</td>
                <td>${clean(q.pieces)}</td>
                <td>${clean(q.total_kg)}</td>
                <td>${clean(q.price_kg)} ريال</td>
                <td>${fmt(subtotal)} ريال</td>
              </tr>
            </tbody>
          </table>

          <div class="a4-totals-terms">
            <div class="a4-terms">
              <h3>الشروط والملاحظات</h3>
              <ul>
                <li>الأسعار أعلاه حسب المواصفات الموضحة في هذا العرض.</li>
            ...
            <div class="a4-total-box">
              <div class="a4-total-row"><span>الإجمالي قبل الضريبة</span><b>${fmt(subtotal)} ريال</b></div>
              <div class="a4-total-row"><span>ضريبة القيمة المضافة 15%</span><b>${fmt(vat)} ريال</b></div>
              <div class="a4-total-row final"><span>الإجمالي النهائي</span><b>${fmt(grand)} ريال</b></div>
            </div>
          </div>

          <div class="a4-approval">
            <div class="a4-sign-box"><b>توقيع العميل</b><div class="a4-sign-line">الاسم والتوقيع</div></div>
            <div class="a4-sign-box"><b>اعتماد المندوب</b><div class="a4-sign-line">${repName(q.rep_id)}</div></div>
            <div class="a4-sign-box"><b>اعتماد مدير المبيعات</b><div class="a4-sign-line">التوقيع</div></div>
            <div class="a4-sign-box"><b>ختم الشركة</b><div class="a4-sign-line">الختم</div></div>
          </div>

          <div class="a4-footer">
            <span>شركة جدة النموذجية للصناعة</span>
            <span>رقم العرض للتحقق: ${clean(q.quote_no)}</span>
            <span>هذا العرض لا يعتبر طلبًا نهائيًا إلا بعد الاعتماد.</span>
          </div>
        </div>
      </div>
    `;
