...
(function(){
  ...
  window.pullCloudData = async function(){
    ...
    try{
      const current = getDb();
      const next = {...current};
      const entries = await Promise.all(Object.keys(tableMap).map(async key=>[key,await pullList(key)]));
      const fingerprint = entries.map(([key,list])=>key+':'+list.map(x=>x._cloud_updated_at||'').join(',')).join('|');
      if(fingerprint && fingerprint === lastCloudFingerprint){
        showCloudStatus("cloud", "البيانات محدثة");
        return;
      }
  ...
  window.pushCloudData = async function(){
    ...
    syncBusy = true;
    try{
      await Promise.all(Object.keys(tableMap).map(upsertList));
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const poll=()=>{
      clearTimeout(window.__jmsCloudPollTimer);
      window.__jmsCloudPollTimer=setTimeout(async()=>{
        if(cloudReady && !document.hidden && navigator.onLine) await pullCloudData();
        poll();
      },120000);
    };
    poll();
    document.addEventListener('visibilitychange',()=>{if(!document.hidden&&cloudReady&&navigator.onLine)pullCloudData();});
  });
})();
