(function(){
  'use strict';
  const VERSION='2026-08-13-professional-experience-2';
  ...
  function applyLaunchRoute(){if(launchRouteApplied||!visible())return;const params=new URLSearchParams(location.search),page=params.get('page'),action=params.get('action');if(!page)return;const nav=document.querySelector(`.nav[data-page="${page}"]`);if(!nav)return;launchRouteApplied=true;nav.click();if(page==='quotes'&&action==='new')setTimeout(()=>window.openQuoteForm?.(),180);}
  ...
  const style=document.createElement('style');style.textContent=`
    ...
    body:has(#jmsInstallAppButton) .jms-quick-menu{bottom:82px}body:has(#jmsInstallAppButton) #jmsGlobalSearch{top:auto;bottom:146px}
