...
export const ACTIONS = ['debts','collections','visits','quotes','customer_summary','attention','daily_plan','order_tracking','quote_draft','general'];
...
export function scopeData(data,auth) {
 ...
 for(const key of ['customers','quotes','orders','visits','collections','manufacturing','storeOrders'])out[key]=(data[key]||[]).filter(r=>String(r.rep_id||'')===String(auth.id));
...
export function executePlan(raw,data,auth,now=new Date()) {
 ...
 return result('وضح المطلوب من بيانات البرنامج: تحليل ديون أو تحصيل، متابعة عميل أو عرض، تجهيز مسودة من عرض سابق، أو تتبع طلب.',{mode:'clarification'});
}
