/* JMS daily route save reliability fix. */
(function(){
  'use strict';
  const VERSION='20260816-route-save-1';
  ...
  function persistRouteFromDom(){
    ...
    if(route){
      ...
      route.source='rep_daily_plan';
    }else{
      route={id:uid(),date:localDate(),rep_id:u.id,items,source:'rep_daily_plan',created_at:new Date().toISOString()};
  ...
  function verifyRoute(expected){
    ...
    if(ok){try{if(typeof save==='function')save()}catch(_){}backupRoutes()}
    return ok;
  }
  function patch(){
    ...
    planner.save=function(){
      const direct=persistRouteFromDom();
      const result=originalSave?.apply(this,arguments);
      setTimeout(()=>{
        if(!verifyRoute(direct)){
          persistRouteFromDom();
        }
        try{window.renderRoutes?.();window.JMSDailyVisitReport?.refresh?.()}catch(_){}
      },100);
      return result;
    };
    ...
    restoreRoutes();
    try{window.renderRoutes?.()}catch(_){}
    return true;
  }
  restoreRoutes();
