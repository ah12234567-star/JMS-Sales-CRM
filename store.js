(function(){
  ...
  async function openCustomerAccount(){
    // Do not prefill a previously saved checkout number in the login dialog.
    // Shared devices otherwise expose or reuse a stale customer's mobile.
    if(!customerToken())return openCustomerLogin('');$('customerAccountBody').innerHTML='<div class="customer-account-content"><h2>حسابي وطلباتي</h2><div class="customer-account-empty">جارٍ تحميل بياناتك...</div></div>';openDialog('customerAccountDialog');
  ...
  document.addEventListener('click',event=>{
    ...
    if(event.target.closest('[data-customer-logout]')){localStorage.removeItem(CUSTOMER_TOKEN_KEY);localStorage.removeItem(CUSTOMER_KEY);syncAccountButton();closeDialog('customerAccountDialog');toast('تم تسجيل الخروج');return}
