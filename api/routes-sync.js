...
export default async function handler(req,res){
  ...
  try{
    if(req.method==='GET'){
      ...
      const items=(rows||[]).map(r=>({...r.data,id:r.data?.id||r.id,_cloud_updated_at:r.updated_at})).filter(r=>allowed(auth,r));
      return json(res,200,{ok:true,items});
    }
    if(req.method==='POST'){
      const body=await readBody(req),items=Array.isArray(body.items)?body.items:[];
      const allowedItems=items.filter(r=>r&&r.id&&allowed(auth,r)).slice(0,500);
