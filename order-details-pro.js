(function(){
  'use strict';
  const VERSION='2026-08-14-order-details-pro-1';
  ...
  const customer=id=>(database().customers||[]).find(item=>item.id===id)||{};
  const rep=id=>(database().reps||[]).find(item=>item.id===id)||(database().users||[]).find(item=>item.id===id)||{};
  const productionFor=id=>(database().productionOrders||[]).find(item=>item.order_id===id);
  ...
  function orderCard(order){
    ...
    return `<article class="jms-order-list-card" role="button" tabindex="0" onclick="jmsOpenOrderDetails('${esc(order.id)}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();jmsOpenOrderDetails('${esc(order.id)}')}"><div class="jms-order-list-top"><div><span>${value(order.order_no||order.date)}</span><h3>${esc(c.name||'عميل غير محدد')}</h3></div><i class="${tone}">${esc(order.status||'جديد')}</i></div><div class="jms-order-list-spec"><span>${value(order.product)}</span><span>${value(order.width)} × ${value(order.length)}</span><span>${value(order.total_kg,' كجم')}</span></div><div class="jms-order-list-foot"><span>${esc(r.name||'مندوب غير محدد')}</span><b>${money(order.amount_value||order.amount)}</b><em>فتح التفاصيل ‹</em></div></article>`;
  }
  function render(){
